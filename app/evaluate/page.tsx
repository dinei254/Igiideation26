...
import { Video, FileText, File, Star, PlusCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function JudgeDashboard() {
  ...
  return (
    <div className="flex h-screen">
      {/* Left Panel */}
      <div className={`bg-muted transition-all duration-300 ease-in-out ${isRightOpen ? 'w-1/2' : 'w-full'} p-4`}>
        ...
        <h1 className="text-2xl font-bold mb-4">{data.project_name} - {data.project_id}</h1>
        {renderContent()}
        </main>
      </div>

      {/* Right Panel */}
      <div className={`bg-background fixed top-0 right-0 h-full transition-all duration-300 ease-in-out ${isRightOpen ? 'w-1/2' : 'w-0'} overflow-hidden`}>
        <div className="p-4">
