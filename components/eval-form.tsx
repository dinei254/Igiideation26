...
export default function EvalForm({ projectId }: { projectId: string }) {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto p-4 border-none h-[100vh] overflow-y-auto">
      <CardHeader>
        <CardTitle className="text-2xl font-bold text-center">Project Evaluation Form</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {questions.map((question, index) => (
          <div key={index} className="space-y-2">
            ...
            <RadioGroup
              onValueChange={(value) => setRatings(prev => ({ ...prev, [question]: value }))}
              className="flex justify-between"
            >
              {[1, 2, 3, 4, 5].map((value) => (
                <div key={value} className="flex flex-col items-center">
                  <RadioGroupItem value={value.toString()} id={`${question}-${value}`} className="peer sr-only" />
                  <Label
        ...
        <div className="space-y-2">
          <Label className="text-lg font-semibold">JURY COMMENTS (IF ANY)</Label>
          <textarea
